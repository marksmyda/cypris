...
export class Cypris {
  ...
  formatCell(value: unknown, col: ColumnDef): string {
    switch (col.type) {
      case 'author':
        return this.getAuthorNames(value as Result['authors']);
      case 'date':
        return this.getDateFromString(value as string);
      default:
        return String(value ?? '');
    }
  }
  ...
  private handleError(err: HttpErrorResponse) {
    if (err.status === 429) {
      this.errorMsg =
        'Too many requests - please pause a moment and try again.';
    } else if (err.status === 500) {
      this.errorMsg =
        'Sorry, the server ran into a problem. Please try again later.';
    } else {
